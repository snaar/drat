...
mod tests {
    ...
    use crate::serde::ser::from_struct::row::to_row;
    use crate::serde::ser::from_struct::typed_header_sink::to_typed_header_sink;
    ...
    fn test() {
        ...
        let sink = to_typed_header_sink(&input_rows, tfn, sink).unwrap();
        assert!(sink.header.is_some());
        assert_eq!(sink.header.unwrap(), expected_header);
        assert_eq!(sink.rows, expected_rows);
    }
}
