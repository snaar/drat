...
impl InputFactory {
    ...
    fn create_source(
        &mut self,
        path: &str,
        file_extension_override: Option<&str>,
    ) -> CliResult<Box<dyn Source>> {
    ...
    fn create_source_from_reader(
        ...
        Err(Error::from(format!(
            "Cannot find source factory for file - {:?}",
            path
        )))
    }
    ...
    fn create_io_reader(&mut self, path: &Path) -> CliResult<Box<dyn io::Read>> {
        ...
        for _factory in &mut self.transport_factories.iter() {
            match _factory.can_open(path) {
                false => continue,
                true => io_reader = Some(_factory.open(path)?),
            }
        }
        match io_reader {
            None => {
                let msg = format!(
                    "Cannot open file {:?}. \
                        Check if the path is valid and/or if a right factory is provided.",
                    &path
                );
